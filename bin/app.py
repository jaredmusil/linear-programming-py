...
class Controller:
    def __init__(self, root):
        ...
        # This does not work on linux PCs
...
class View:
    ...
    def tab_variables(self):
        ...
        # frames
        tab.vars = ttk.LabelFrame(tab, text="Variables")
        tab.lbl2 = ttk.LabelFrame(tab, text="Formula")
        tab.lbl3 = ttk.LabelFrame(tab, text="Standard")
        tab.lbl4 = ttk.LabelFrame(tab, text="Agumented")
        #tab.inputs.grid(row=1, column=0, sticky=tk.NSEW, padx=(0,5), pady=5)


        # widgets
        tab.vars.addBtn = ttk.Button(tab.vars, command=lambda: self.add_variable(tab), image=self.icon.imgAdd)
        tab.vars.removeBtn = ttk.Button(tab.vars, command=lambda: self.remove_variable(tab), image=self.icon.imgRemove)
        tab.vars.label1Label = ttk.Label(tab.vars, text='Label')
        tab.vars.variable1Label = ttk.Label(tab.vars, text='Var')
        tab.vars.label1Entry = ttk.Entry(tab.vars, textvariable='label1')
        tab.vars.variable1Entry = ttk.Entry(tab.vars, textvariable='variable1')

        tab.lbl2.label1Entry = ttk.Entry(tab.lbl2, textvariable='label1')
        tab.lbl3.label1Entry = ttk.Entry(tab.lbl3, textvariable='label1')
        tab.lbl4.label1Entry = ttk.Entry(tab.lbl4, textvariable='label1')

        # frame widget placement
        tab.vars.grid(row=0, column=0, sticky=tk.NSEW, padx=5, pady=5)
        tab.lbl2.grid(row=1, column=0, sticky=tk.NSEW, padx=5, pady=5)
        tab.lbl3.grid(row=2, column=0, sticky=tk.NSEW, padx=5, pady=5)
        tab.lbl4.grid(row=3, column=0, sticky=tk.NSEW, padx=5, pady=5)
        tab.vars.addBtn.grid(row=0, column=0, sticky=tk.NSEW)
        tab.vars.removeBtn.grid(row=0, column=1, sticky=tk.NSEW)

        tab.vars.label1Label.grid(row=1, column=0, sticky=tk.NSEW)
        tab.vars.variable1Label.grid(row=1, column=1, sticky=tk.NSEW)
        tab.vars.label1Entry.grid(row=2, column=0, sticky=tk.NSEW)
        tab.vars.variable1Entry.grid(row=2, column=1, sticky=tk.NSEW)

        tab.lbl2.label1Entry.grid(row=1, column=0, sticky=tk.NSEW)
        tab.lbl3.label1Entry.grid(row=1, column=0, sticky=tk.NSEW)
        tab.lbl4.label1Entry.grid(row=1, column=0, sticky=tk.NSEW)

        # frame widget window resizing
        tab.vars.grid_columnconfigure(0, weight=1)
        tab.vars.grid_rowconfigure(0, weight=1)
    ...
    def tab_standard(self):
        ...
        # frame widgets
        tab.addRemove.addBtn = ttk.Button(tab.addRemove, command=lambda:self.open_directory(tab, 'changeME'), image=self.icon.imgAdd)
        tab.addRemove.removeBtn = ttk.Button(tab.addRemove, command=lambda:self.open_directory(tab, 'changeME'), image=self.icon.imgRemove)
